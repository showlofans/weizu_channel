...
public class ChargeRecordAoImpl implements ChargeRecordAO {
	...
	public int updateAccount(ChargeRecordPo chargeRecordPo,ChargeAccountPo accountPo) {
		...
		/****************修改登陆账户********************/
//		ChargeAccountPo accountPo = chargeAccountDao.selectByAgencyId(agnecyId);
		/**充值前余额*/
		...
		chargeRecordDao.add(new ChargeRecordPo(System
				...
				agencyBeforeBalance, NumberTool.add(agencyBeforeBalance, (-1)*chargeAmount), 
		...
		/** 数据库取出来的的账户对象 */
		ChargeAccountPo chargeAccountPo = chargeAccountDao
				.get((long) accountId);
		/**充值前余额*/
