...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 话费通道列表 <span class="c-gray en">&gt;</span> 话费折扣列表 <span class="c-gray en">&gt;</span>绑定代理商 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.reload();" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a> </nav>
<div class="page-container">
	...
	<div class="text-c">
		...
		<c:forEach items="${resultMap.onameEnums }" var="oname" varStatus="vs1">
			 <c:if test="${telChannelParams.operatorName == oname.value }">
				${oname.desc }
			</c:if>	
		</c:forEach>
		<c:forEach items="${resultMap.billTypeEnums }" var="billEnum" varStatus="vs">
			<span id="billTypeDesc"  class="c-red">
			<c:if test="${resultMap.telrateBindAccountVO.billType==billEnum.value }">${billEnum.desc }</c:if>
			</span>
		</c:forEach>
		<span id="billTypeDesc"  class="c-red">
			${resultMap.telrateBindAccountVO.activeDiscount }  
		</span>
	...
	<div class="text-c">
		<form action="/flowsys/telRate/batch_bindtel_agency_page.do" method="post" id="formD" name="dataListForm">
				...
				绑定状态：<span class="select-box inline">
						...
						<c:forEach items="${resultMap.bindStateEnums }" var="bState" varStatus="vst">
							<option value="${bState.value }" <c:if test="${bState.value == resultMap.telrateBindAccountVO.bindState }"> selected</c:if>>${bState.desc }</option>
						</c:forEach>
					</select>
				</span>
				
				代理商名称:<input type="text"  value="${resultMap.telrateBindAccountVO.agencyName }" name="agencyName" id="" placeholder=" 代理商名称" style="width:150px" class="input-text">
				...
				<input type="hidden" id="telRateId" name="telRateId" value="${resultMap.telrateBindAccountVO.telRateId }"> 
		</form>
	</div>
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			<thead>
				<tr class="text-c">
					...
					<th width="60">带票费率</th> -->
					<!-- <th width="120">操作</th> -->
					<c:if test="${resultMap.telrateBindAccountVO.bindState==1  }">
						<th width="120">解绑时间</th>
					</c:if>
					<%-- <c:choose>
						<c:when test="${resultMap.telrateBindAccountVO.bindState==1  }">|| resultMap.telrateBindAccountVO.bindState==null
							<th width="120">解绑时间</th>
						</c:when>
						<c:otherwise>
			...
			<tbody>
				<c:forEach items="${resultMap.pagination.records }" var="agency" varStatus="vs">
					<tr class="text-c font-red">
						...
						<c:if test="${resultMap.telrateBindAccountVO.bindState==1  }"><td>${agency.createTimeStr }</td></c:if>
...
function changeBAllState(url,bindState, updateBindState){
	...
	$.ajax({
		...
		url: url,
		//dataType: 'json',
		data: {updateBindState:updateBindState,telRateId:telRateId,billType:billType,agencyName: agencyName,agencyMark: agencyMark,bindState: bindState},
...
function changeBState(url,bindState){
	...
	if(accountIds.length > 1) //如果获取到
    {
    	...
    	$.ajax({
			...
			url: url,
			//dataType: 'json',
			data: {bindState: bindState,telRateId:telRateId,accountIds:accountIds},
			success: function(resp){
				//$(obj).parents("tr").remove();
				//alert
				if(resp=="success"){
