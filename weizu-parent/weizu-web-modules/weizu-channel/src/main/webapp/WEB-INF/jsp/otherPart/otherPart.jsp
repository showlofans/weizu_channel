...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th colspan="7" scope="col">服务器文件系统</th>
			</tr>
			<tr class="text-c">
				<th>磁盘</th>
				<th>总大小</th>
		...
		<tbody>
			<c:forEach var="file" varStatus="vs" items="${resultMap.systemFileList }">
				<tr class="text-c">
					<td>${file.devName }</td>
					<td>${file.totalSize }</td>
					<td>${file.freeSize }</td>
					<td>${file.availSize }</td>
					<td>${file.usedSize }</td>
					<td>${file.usePercent }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	<table class="table table-border table-bordered table-bg mt-20">
		<thead>
			<tr>
				<th colspan="2" scope="col">今日笑话</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach var="jokeContentMap" varStatus="vs" items="${resultMap.jokeContentMapList }">
				<c:if test="${vs.index < 3 }">
				<tr class="text-c">
					<th class="pd-10" style="background-color:#e5e5e5; border:solid 1px #ddd" width="10%">${jokeContentMap.updatetime }</th>
					<td class="pd-10" style="background-color:#e5e5e5; border:solid 1px #ddd">
			            <div style=" background-color:#fff">${jokeContentMap.content }</div>
					</td>
				</tr>
				</c:if>
			</c:forEach>
			<%-- <tr>
				<th width="30%">${resultMap.jokeContentMap.updatetime }</th>
				<td><span id="lbServerName">${resultMap.jokeContentMap.content }</span></td>
			</tr> --%>
			<!-- <tr>
				<td>服务器IP地址</td>
				<td>192.168.1.1</td>
