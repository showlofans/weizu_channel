	...
	<form action="" method=""  class="form form-horizontal" id="platfrom">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal">
				<c:forEach items="${resultMap.epEncodeTypeEnums }" var="epEncodeTypeEnum" varStatus="vs">
					<div class="radio-box">
						<input name="epEncodeType" class="radioItem" type="radio" id="epEncodeType-${vs.index }" value="${epEncodeTypeEnum.value }"<c:if test="${vs.index==0 }">checked</c:if>>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" onblur="checkEpName(this)" placeholder="" id="epName" name="epName">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>平台英文标识：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="epEngId" name="epEngId">
			</div>
		</div>
...
function checkEpName(vart){
	...
	$.ajax({
        type:"post",
        url:"/flowsys/platform/check_ep_name.do",
